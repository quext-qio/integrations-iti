...
class DataControllerFactory:
    ...
    def create_data_controller(self, input):
        ...
        if partner == "Newco":
            property_data, models_data, units_data, code = DataNewco().get_unit_availability(ips_response, input)
            return Controller("NewCo", code).built_response(property_data, models_data, units_data)    
        elif partner == "ResMan":
            property_data, models_data, units_data, code = DataResman().get_unit_availability(ips_response, input)
