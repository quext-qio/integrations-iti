...
pricing_info as (
  ...
  from 
    (
      ...
      from 
        prop_select 
        join units on units.property_id = prop_select.id 
        ...
        left join amenities_unit on units.id = amenities_unit.unit_id
...
moving_info as (
  ...
  from 
    ...
    left join (
      ...
      from 
        prop_select 
        join resident_events re_out on prop_select.id = re_out.property_id 
        and re_out.deleted_at is null 
        and re_out.eventable_type = 'App\\\\MoveOutEvent' 
        join move_out_events moe on re_out.eventable_id = moe.id_move_out_events 
        and moe.deleted_at is null
    ) move_out_info on prop_select.id = move_out_info.property_id 
    ...
    left join (
      ...
      from 
        prop_select 
        join resident_events re on prop_select.id = re.property_id 
        and re.deleted_at is null 
        and re.eventable_type = 'App\\\\NoticeEvent' 
        join notice_events ne on re.eventable_id = ne.id_notice_events 
        and ne.deleted_at is null
    ) notice_info on prop_select.id = notice_info.property_id 
...
unit_move_info as (
  select 
    ...
    coalesce(
      moving_info.moveout_date, moving_info.estimated_moveout_date
    ) as moveout_date, 
    lead (movein_date) over (
      partition by prop_select.id, 
      unit_id 
      order by 
        movein_date
    ) as next_movein_date 
  from 
    prop_select 
    inner join residents on residents.property_id = prop_select.id 
    and residents.deleted_at is null 
    left join moving_info on prop_select.id = moving_info.property_id
...
unit_move_info_all as (
  select 
    ...
    movein_date, 
    moveout_date, 
    case when next_movein_date = movein_date then null else next_movein_date end as next_movein_date, 
    row_number() over (
  ...
  from 
    prop_select 
    join unit_move_info umi on umi.prop_id = prop_select.id 
    join pricing_info pi on pi.unit_ids = umi.unit_id 
...
/* the unit is a bad unit */
    (
      ...
      and units.`status` in (
        '', 'Inactive', 'Demo', 'Model', 'Shop', 
        'Down', 'Rehab', 'Pending Approval'
      )
    ) 
    or
/* the unit's available date has been manually set to sometime beyond 60 days */
    units.available_date > date_add(
      curdate(),
      interval 60 day
    )
    or (
...
/* someone is moving in and will do so anytime in the next 365 days */
  ...
  from 
    ...
    join units on units.property_id = prop_select.id 
    and units.deleted_at is null 
    left join unit_move_info_all umia on units.id = umia.unit_id 
    and rn = 1 
...
applicant_info as (
  ...
  from 
    prop_select 
    join unit_move_info_all umia on umia.prop_id = prop_select.id 
    join unit_leases on unit_leases.resident_id = umia.resident_id 
    and unit_leases.unit_id = umia.unit_id 
    join applicants on prop_select.id = applicants.property_id 
    and applicants.unit_id = umia.unit_id 
    and applicants.status in('active', 'moved in') 
    and applicants.deleted_at is null 
    join persons on applicants.person_id = persons.id 
  where 
...
/* , case when available_date <= curdate() then curdate() else umic.available_date end as avldt */
    ...
    and available_date <= curdate() then curdate() when available = 0 
    and available_date <= curdate() then null else available_date end as available_date, 
    ...
    unit_market_rent, 
    min(unit_market_rent) over (
      partition by umic.prop_id, unit_types.`name`
    ) as unit_market_rent_all, 
    case when unit_types.`description` is null then case when umic.bedrooms = 0 then 'Studio' else concat(
      trim(umic.bedrooms)+ 0, 
      'x', 
      trim(umic.bathrooms)+ 0
    ) end else unit_types.`description` end as unit_type_desc, 
    min(umic.square_feet) over (
      partition by umic.prop_id, unit_types.`name`
    ) as sqft_min_all, 
    max(umic.square_feet) over (
      partition by umic.prop_id, unit_types.`name`
    ) as sqft_max_all, 
    trim(unit_types.square_feet)+ 0 as sqft_model_base, 
    case when nullif(website_floorplans.image, '') is not null then concat(
      'https://madera-newco.s3.us-west-2.amazonaws.com/', 
      website_floorplans.image
    ) else null end as `floorplan`, 
    websites.url `website`, 
    case when website_floorplans.virtual_tour_url not in (null, 'undefined') then website_floorplans.virtual_tour_url end as virtual_tour_url 
  from 
    prop_select 
    left join unit_move_info_calc umic on umic.prop_id = prop_select.id 
    join unit_types on unit_types.id = umic.unit_type_id 
	-- and unit_types.property_id = prop_select.id 
    join pricing_info on pricing_info.unit_ids = umic.unit_id 
    left join buildings on buildings.id = building_id 
    left join website_floorplans on website_floorplans.unit_type_id = unit_types.id 
    left join websites on websites.property_id = prop_select.id 
  where 
    prop_id is not null
) 
/*
...
unit_report_avail as (
  select 
    ...
    unit_market_rent, 
    min(unit_market_rent) over (
      partition by prop_id, unit_types.`name`
    ) as unit_market_rent_avail, 
  ...
  from 
    prop_select 
    join unit_move_info_calc umic on umic.prop_id = prop_select.id 
    join unit_types on unit_types.property_id = prop_select.id 
    and unit_types.id = umic.unit_type_id 
    and umic.unit_status <> 'Pending Approval' 
    join pricing_info on umic.unit_id = pricing_info.unit_ids 
    left join applicant_info ai on ai.prop_id = prop_select.id 
...
unit_report_consolidated as (
  select 
    ...
    min(ura2.unit_market_rent) over (
      partition by prop_id, ura1.unit_type_name
    ) as unit_market_rent_avail,
  ...
  from 
    ...
    left join unit_report_avail ura2 on ura1.prop_id = ura2.prop_id 
