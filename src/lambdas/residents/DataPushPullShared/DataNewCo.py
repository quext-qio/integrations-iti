...
class DataNewco:
    ...
    def get_resident_data(self, ips, movein, moveout, community_id):
        errors = []
        ...
        if movein != None:         
            newcoResidentsQuery += "AND STR_TO_DATE(moveInDate, '%Y-%m-%d') >= :move_in_date "

        if moveout != None:
            newcoResidentsQuery += "AND (STR_TO_DATE(moveOutDate, '%Y-%m-%d') <= :move_out_date OR moveOutDate IS NULL) "
        ...
        with closing(self.outgoing.sql.get('Newco DB (Internal)').session()) as session:
            ...
            parameters = { "newco_property_id": ips["platformData"]["communityID"],
                           "move_in_date": movein,
                           "move_out_date": moveout
            }

            # Get the charge codes data...
            result = session.execute(newcoResidentsQuery, parameters)
            residents = []
            for row in result:
                rowdict = dict(row.items())
                if rowdict['SSN'] is not None:
                    ssn = decrypt(str(rowdict['SSN']), database_key['newco_key'])
                    rowdict["SSN"] = ssn.decode(encoding='UTF-8') if ssn else None
                    date_val = str(datetime.datetime.now())
                    enc_str = community_id + date_val + os.environ['SECRET_KEY_SSN']
                    rowdict["timestamp_enc"] = date_val
                    if rowdict["SSN"]:
                        rowdict["SSN"] = encrypt(str(rowdict['SSN']), enc_str)
                residents.append(rowdict)

        response = { "data": { "provenance": [ "newco-db" ], "residents": residents }, "errors": errors }
