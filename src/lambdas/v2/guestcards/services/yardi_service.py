...
class YardiService(ServiceInterface):
    ...
    def get_data(self, body: dict, ips_response: dict, logger):
        ...
        if "tourScheduleData" in body:
            ...
            if appointment_date != "":
                ...
                if code != 200:
                    ...
                    tour_requested = appointment_date
                    tour_scheduled_id = quext_response.get(
                        "data", {}).get("id", "")
                    ...
                    if purpose_exists:
                        ...
                        if unit_id is None:
                            event_list = self.get_events(event_object=event_object, date_tour=tour_requested[0: tour_requested.index(
                                "T")] + "T" + formatted_time, tour_comment=tour_comment, unit_id=None)
                        else:
                            event_list = self.get_events(event_object=event_object, date_tour=tour_requested[0: tour_requested.index(
        ...
        try:
            ...
            messages = response.get('soap:Envelope', {}).get('soap:Body', {}).get('ImportYardiGuest_LoginResponse', {
            }).get('ImportYardiGuest_LoginResult', {}).get('Messages', {}).get('Message', [])
    ...
    def get_yardi_partner_id(self):
        ...
        partner_uuid = next(
            (partner for partner in partners_info["content"] if partner['name'] == 'Yardi'), None)
  ...
  # Define function to generate unique ID from three UUIDs
        ...
        try:
            ...
            for ils_unit in ils_units:
                ...
                if (int(float(ils_unit.find('.//MarketRent').text)) <= int(desired_rent)) and (int(float(ils_unit.find('.//UnitBedrooms').text)) >= int(num_bedrooms)):
                    unit_economic_status = ils_unit.find(
                        './/UnitEconomicStatus').text
                    unit_occupancy_status = ils_unit.find(
                        './/UnitOccupancyStatus').text
                    if unit_occupancy_status == "vacant":
                        if unit_economic_status in ["model", "residential"]:
                            identification_element = ils_unit.find(
                                './/Identification')
   ...
   # Add Guided  tour event
    def get_events(self, event_object, date_tour, tour_comment, unit_id: None):
        ...
        new_date = event_object["@EventDate"][0:
                                              event_object["@EventDate"].index("T")+1]
