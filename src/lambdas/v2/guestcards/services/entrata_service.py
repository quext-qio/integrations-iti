...
class EntrataService(ServiceInterface):
    def get_data(self, body: dict, ips_response: dict, logger):
        ...
        data = self.__get_from_payload_for_guestcards(
            body, ips_response, tour, tour_comment)
        request_body = {
            "auth": {
                "type": "basic"
            },
        ...
        response = requests.post(outgoing_entrata, data=json.dumps(
            request_body), params=_params, headers=_headers)
        # Checking for Empty list
        response = json.loads(response.text)
        response_body = response[EntrataConstants.RESPONSE][EntrataConstants.RESULT][EntrataConstants.PROSPECTS][EntrataConstants.PROSPECT][
            0] if EntrataConstants.RESULT in response[EntrataConstants.RESPONSE] else response[EntrataConstants.RESPONSE]
        ...
        if EntrataConstants.ERROR in response[EntrataConstants.RESPONSE] or (EntrataConstants.CODE in response[EntrataConstants.RESPONSE] and response[EntrataConstants.RESPONSE][EntrataConstants.CODE] != 200):
            error = response[EntrataConstants.RESPONSE][EntrataConstants.ERROR][EntrataConstants.MESSAGE] if EntrataConstants.ERROR in response[
                EntrataConstants.RESPONSE] else response_body[EntrataConstants.MESSAGE]
            return {
    ...
    def __get_from_payload_for_guestcards(self, payload, ips, tour_info, tour_comment=None):
        ...
        # Default value if not specified in the payload
        desired_bathroom = customer_preference.get(
            EntrataConstants.DESIRED_BATHS, 0)
        desired_rent = customer_preference.get(
            EntrataConstants.DESIRED_RENT, 0)
        occupants = customer_preference.get(EntrataConstants.NO_OCCUPANTS, 0)
        lease_term_months = customer_preference.get(
            EntrataConstants.LEASE_TERMS, 0)
        ...
        event_reason_id = ips[EntrataConstants.PARAMS][EntrataConstants.EVENTREASON_ID] if (
                EntrataConstants.EVENTREASON_ID in ips[EntrataConstants.PARAMS] and
                ips[EntrataConstants.PARAMS][EntrataConstants.EVENTREASON_ID] != 0) \
        ...
        param_dict = {
            "propertyId": ips[EntrataConstants.PARAMS][EntrataConstants.FOREIGN_COMMUNITY_ID],      #["foreign_community_id"],
            EntrataConstants.PROSPECTS: {
                EntrataConstants.PROSPECT: {
                    "leadSource": {
                        "originatingLeadSourceId": ips[EntrataConstants.PARAMS][EntrataConstants.LEADSOURCE_ID]
                    },
    ...
    def get_entrata_available_times(self, ips, date_tour, logger):
        ...
        tour_start = datetime.strptime(
            date_tour["start"], "%Y-%m-%dT%H:%M:%SZ")
        output_timestamp = tour_start.strftime("%m/%d/%Y")
        # Getting parameter from payload
        _body = {
            EntrataConstants.PROPERTYID: ips[EntrataConstants.PARAMS][EntrataConstants.FOREIGN_COMMUNITY_ID],
        ...
        for value in res[EntrataConstants.AVAILABLE_HOURS][EntrataConstants.AVAILABLE_HOUR]:
            start_time = value[EntrataConstants.DATE] + \
                ' '+value[EntrataConstants.START_TIME]
            end_time = value[EntrataConstants.DATE] + \
                ' '+value[EntrataConstants.END_TIME]
        ...
        for items in res[EntrataConstants.AVAILABLE_HOURS][EntrataConstants.AVAILABLE_HOUR]:
            starttime = items[EntrataConstants.DATE] + \
                'T' + items[EntrataConstants.START_TIME]
            endtime = items[EntrataConstants.DATE] + \
                'T' + items[EntrataConstants.END_TIME]
            time_slot.append(
                {EntrataConstants.START_TIME: starttime, EntrataConstants.END_TIME: endtime})
