

class NewcoConstants:

    NEWCO_EXTERNAL = "Newco DB (Internal)"
    AVAILABLE = "available"
    FILTER_ON_AVAILABILITY = "filter_on_availability"
    MARKET_RATE_UNIT_TYPE_AVAIL = "market_rate_unit_type_avail"
    MARKET_RENT_UNIT_TYPE = "market_rent_unit_type"
    MARKET_RENT = "market_rent"
    BUILTIN = "builtin"
    NEWCO_QUERY_CACHE = 'Newco_Query_cache'
    IS_AVAILABLE = ' having available = 1'
    NEWCO_PROPERTY_QUERY = 'newco_property_query'
    NEWCO_MODELS_QUERY = 'newco_models_query'
    NEWCO_UNITS_QUERY = 'newco_units_query'
    NEWCO_BASE_QUERY = 'newco_base_query'
    QUERY_PATH = '/NewcoShared/Utilities/SQL/'
    NEWCO_PROPERTY_ID = "newco_property_id"
    PLATFORMDATA = "platformData"
    COMMUNITYID = "communityID"
    NEWCO_UNITS_PROPERTY_QUERY = "NewcoUnitsPropertyQuery.sql"
    NEWCO_UNITS_MODELS_QUERY = "NewcoUnitsModelsQuery.sql"
    NEWCO_UNITS_UNITS_QUERY = "NewcoUnitsUnitsQuery.sql"
    NEWCO_UNITS_BASE_QUERY = "NewcoUnitsBaseQuery.sql"
    SOURCESYSTEM = "Newco"
